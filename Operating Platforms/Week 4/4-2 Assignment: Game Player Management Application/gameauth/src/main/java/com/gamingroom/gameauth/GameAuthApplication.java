...
import com.gamingroom.gameauth.GameAuthResource; // imported resource class - Marc Aradillas
...
public class GameAuthApplication extends Application<Configuration> {
	...
	public void run(Configuration c, Environment e) throws Exception 
	{
		...
		// Register GameUserRESTController (based on BasicAuth Security Example) - Marc Aradillas
		e.jersey().register(new GameUserRESTController(e.getValidator()));
		
		// Create io.dropwizard.client.JerseyClientBuilder instance and give it io.dropwizard.setup.Environment reference (based on BasicAuth Security Example) - Marc Aradillas
		final Client client = new JerseyClientBuilder(e).build("DemoRESTClient");
	    e.jersey().register(new RESTClientController(client));
	    
	    // reference: https://howtodoinjava.com/dropwizard/dropwizard-basic-auth-security-example/
	    // Configure BasicCredentialAuthFilter - Marc Aradillas
