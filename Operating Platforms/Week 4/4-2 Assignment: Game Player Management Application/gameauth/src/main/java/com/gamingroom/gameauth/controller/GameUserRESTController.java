...
// added path annotation for gameusers, This will specify the base path for all the endpoints in this class to be /gameusers - Marc Aradillas
@Path("/gameusers")
...
public class GameUserRESTController {
    ...
    @GET
    @Path("/{id}")
    // Added RolesAllowed annotation for USER based on BasicAuth Security Example - Marc Aradillas
    @RolesAllowed("USER")
    ...
    // Added RolesAllowed annotation for ADMIN based on BasicAuth Security Example - Marc Aradillas
    @RolesAllowed("ADMIN")
    @POST
